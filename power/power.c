...
static void file_write(char *path, char *s)
{
    char buf[80];
    int len;
...
static void power_set_interactive(struct power_module *module, int on)
{
    ...
    {
      if(strcmp(file_read("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq"), "1400000") != 0)
        if(strcmp(file_read("/data/.power"), "off") == 0)
          {
          file_write("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq", "1400000");
          file_write("/data/.power", "on");
          }
      else
        return;
    } else // if device is going to sleep
    {
      if(strcmp(file_read("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq"), "500000") != 0)
        if(strcmp(file_read("/data/.power"), "on") == 0)
          {
          file_write("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq", "500000");
          file_write("/data/.power", "off");
          }
      else
        return;
    }
    char *governor = file_read("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor");
