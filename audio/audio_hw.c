...
struct m0_audio_device {
    ...
    audio_mode_t mode;
    int active_out_devices;
    int out_device;
    int active_in_devices;
    int in_device;
...
void select_devices(struct m0_audio_device *adev)
{
    ...
    ALOGV("Changing output devices %x => %x\n", adev->active_out_devices, adev->out_device);
    ALOGV("Changing input devices %x => %x\n", adev->active_in_devices, adev->in_device);

    /* Turn on new devices first so we don't glitch due to powerdown... */
    for (i = 0; i < adev->num_dev_cfgs; i++)
    if ((adev->out_device & adev->dev_cfgs[i].mask) &&
        !(adev->active_out_devices & adev->dev_cfgs[i].mask))
        set_route_by_array(adev->mixer, adev->dev_cfgs[i].on,
                   adev->dev_cfgs[i].on_len);

    for (i = 0; i < adev->num_dev_cfgs; i++)
    if ((adev->in_device & adev->dev_cfgs[i].mask) &&
        !(adev->active_in_devices & adev->dev_cfgs[i].mask))
        set_route_by_array(adev->mixer, adev->dev_cfgs[i].on,
                   adev->dev_cfgs[i].on_len);
