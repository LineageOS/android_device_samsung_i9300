...
static void end_call(struct m0_audio_device *adev)
{
    ...
    for (channel = 0; channel < 2; channel++) {
        mixer_ctl_set_value(adev->mixer_ctls.aif2dacl_source, channel, 0);
        mixer_ctl_set_value(adev->mixer_ctls.aif2dacr_source, channel, 1);
        mixer_ctl_set_value(adev->mixer_ctls.aif2_mode, channel, 0);
        mixer_ctl_set_value(adev->mixer_ctls.dac1l_mixer_aif2_switch, channel, 0);
        mixer_ctl_set_value(adev->mixer_ctls.dac1r_mixer_aif2_switch, channel, 0);
        mixer_ctl_set_enum_by_string(adev->mixer_ctls.aif2dac_mux, "AIF3DACDAT");
    }
...
static void select_output_device(struct m0_audio_device *adev)
{
    ...
    /* select input path if in a call */
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        if (!bt_on) {
        ...
        switch(adev->devices & AUDIO_DEVICE_OUT_ALL) {
            ...
            case AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET:
            case AUDIO_DEVICE_OUT_BLUETOOTH_SCO_CARKIT:
                LOGD("%s: AUDIO_DEVICE_OUT_BLUETOOTH_SCO", __func__);
                for (channel = 0; channel < 2; channel++) {
                    // enable bluetooth route
                    mixer_ctl_set_value(adev->mixer_ctls.dac1l_mixer_aif2_switch, channel, 1);
                    mixer_ctl_set_value(adev->mixer_ctls.dac1r_mixer_aif2_switch, channel, 1);
                }
                break;
            default:
                LOGD("%s: DEFAULT OUTPUT", __func__);
                for (channel = 0; channel < 2; channel++) {
                    // enable voicecall route
                    mixer_ctl_set_value(adev->mixer_ctls.aif2dacl_source, channel, 0);
                    mixer_ctl_set_value(adev->mixer_ctls.aif2dacr_source, channel, 0);
                    mixer_ctl_set_value(adev->mixer_ctls.aif2_mode, channel, 0);
                    mixer_ctl_set_value(adev->mixer_ctls.dac1l_mixer_aif1_switch, channel, 1);
                    mixer_ctl_set_value(adev->mixer_ctls.dac1r_mixer_aif1_switch, channel, 1);
                    mixer_ctl_set_value(adev->mixer_ctls.dac1l_mixer_aif2_switch, channel, 1);
                    mixer_ctl_set_value(adev->mixer_ctls.dac1r_mixer_aif2_switch, channel, 1);
                    mixer_ctl_set_enum_by_string(adev->mixer_ctls.aif2dac_mux, "AIF2DACDAT");
                }
                break;
        }
    }
...
static void select_input_device(struct m0_audio_device *adev)
{
    ...
    switch(adev->devices & AUDIO_DEVICE_OUT_ALL) {
        case AUDIO_DEVICE_OUT_EARPIECE:
        case AUDIO_DEVICE_OUT_SPEAKER:
            adev->devices &= ~AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET;
            adev->devices &= ~AUDIO_DEVICE_IN_WIRED_HEADSET;
...
static int start_input_stream(struct m0_stream_in *in)
{
    ...
    in->config.rate = DEFAULT_IN_SAMPLING_RATE;
