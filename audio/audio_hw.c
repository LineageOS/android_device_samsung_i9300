...
#define CAPTURE_PERIOD_SIZE   160
...
static void select_input_device(struct m0_audio_device *adev)
{
    ...
    if (adev->mode != AUDIO_MODE_IN_CALL) {
        for (channel = 0; channel < 2; channel++) {       
            mixer_ctl_set_value(adev->mixer_ctls.aif2_mode, channel, 1);
            mixer_ctl_set_value(adev->mixer_ctls.dac1l_mixer_aif1_switch, channel, 1);
            mixer_ctl_set_value(adev->mixer_ctls.dac1r_mixer_aif1_switch, channel, 1);
            mixer_ctl_set_value(adev->mixer_ctls.dac1l_mixer_aif2_switch, channel, 0);
            mixer_ctl_set_value(adev->mixer_ctls.dac1r_mixer_aif2_switch, channel, 0);
    ...
    if (!bt_on) {
        ...
        if (!sub_mic_on) {
            adev->devices = AUDIO_DEVICE_IN_WIRED_HEADSET | AUDIO_DEVICE_IN_BUILTIN_MIC;
        }
    }
...
static int start_input_stream(struct m0_stream_in *in)
{
    ...
    in->config.rate = DEFAULT_OUT_SAMPLING_RATE;
